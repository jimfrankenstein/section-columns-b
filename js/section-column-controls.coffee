...
leadpagesUX.sectionColumnControls =
  ...
  toggleOption: (option,optionContainer) ->
    ...
    close = true if option.classList.contains('isOpen')

    try
      optionContainer.querySelector('.stylesPane__option.isOpen').classList.remove('isOpen')

    option.classList.add('isOpen') unless close
